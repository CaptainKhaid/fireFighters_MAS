...
public class Forester {
	 ...
	 * 
	 * @param context
	 *            - context to which the forester is added
	 * @param grid
	 *            - grid to which the forester is added
	 * @param id
	 *            - an ID
	 */
	...
	public void step() {
		if (contacts.isEmpty()) {
			addAllContacts();
		} // Assumes that all of the foresters know each other
	...
	private void moveOrExtinguish() {
		...
		if (direction >= 0) {
			if (distance == 0) // If fire is adjacent to or on the cell of the
								// forester
			{
				tryToMove(oppositeDirection);
				return;
			} else if (distance == extinguishingDistance) // If fire is exactly
															// at the
	...
	private boolean tryToMove(int pDir) {
		int[] directions = { pDir, (pDir + 1) % 8, (pDir - 1) % 8, (pDir + 2) % 8, (pDir - 2) % 8, (pDir + 3) % 8,
				(pDir - 3) % 8, (pDir + 4) % 8 };
	...
	private void standingInTheFire() {
		lifePoints--;
		int direction = getOppositeWindDirection(); // Check there the wind
													// blows from

		if (direction > -1) {
			tryToMove(direction);
		} // Try to move in a direction off-the-wind to escape from fire
	}
	...
	private void addAllContacts() {
		...
		for (Object object : objects) {
			if (object.getClass() == Forester.class) {
				contacts.add((Forester) object);
			}
		}
	}
	 ...
	 * 
	 * @param dir
	 *            - direction to extinguish fire in
	 * @param dist
	 *            - distance at which to extinguish
	 */
	private void extinguishFire(int dir, int dist) {
		...
		for (int i = 0; i < dist; i++) {
			if (i == 0) {
				result = Tools.dirToCoord(dir, x, y);
			} else {
				result = Tools.dirToCoord(dir, otherX, otherY);
			}
	 ...
	 * @param y
	 *            - Y coordinate of the fire
	 * @return null - if no fire is at the given cell, obj - if this Fire obj
	 *         was found
	 */
	 ...
	 *         couldn't move because another forester already took this place
	 */
	private int move(int dir) {
		...
		} else if (checkForester(otherX, otherY)) {
			return 1;
		}
	...
	private int[] findDirection2NearestFire() {
		...
		for (Position p : knowledge.getAllFire()) // For all the fires in the
													// forester's knowledge
		{
			...
			// Determine if the fire is closest. If so, update distance and
			...
			if (chebyshevDist < distance) {
				...
				} else if (x - xF == 0) // It's either to the North or to the
										// South
				{
					if (y - yF < 0) {
						direction = 0;
	...
	public boolean checkForester(int x, int y) {
		if (Tools.isWithinBorders(x, y, grid)) {
			...
			for (Object obj : objects) {
				if (obj.getClass() == Forester.class) {
					return true;
				}
			}
		}
	...
	public void recieveMessage(Message message) {
		compareKnowledge(message.getKnowledge());
	}

	/**
	 * Update the knowledge of the forester by taking info from a given
	 * knowledge
	 * 
	 * @param k
	...
	/**
	 * Add a new fire to the forester's knowledge Additional specification is
	 * done for the fire discovered himself
	 * 
	 * @param x
	 *            - X coordinate
	 * @param y
	 *            - Y coordinate
	 * @param foundHimself
	 *            - flag if the fire was found by the forester himself
	 */
	 ...
	 * @return
	 */
	private int getOppositeWindDirection() {
		return (windDirection + 4) % 8;
	}
